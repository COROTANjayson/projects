<script lang="ts">
	...
	import { getHeadlineList, triggerToast } from '$lib/stores/ItemListstore';
	...
	let super_form = superForm(data.headlineForm, {
		onSubmit() {
			isFormLoading = true;
		},
		async onResult({ result }) {
			if (result.status === 200) {
				...
				if (add) {
					triggerToast('add', 'Added new headline');
				} else if (update) {
					triggerToast('update', 'Updated a headline item');
				}
				isFormLoading = false;
	...
	$: {
		if ($headlineData && $editForm) {
			form.set($headlineData);
		} else {
			form.set(data.headlineForm.data);
		}
	}
...
<div class="fixed inset-y-0 top-0 right-0 z-20 flex h-full">
	{#if $sideBarRight && $formName === 'Headline'}
		<div
		...
		>
			<form
				...
				action={$editForm ? '/dashboard/headlines?/edit' : '/dashboard/headlines?/add'}
			>
				<div
				...
				>
					{#if $editForm}
						<div class="space-y-1">
							...
							<h1 class="text-gray-900 text-xl">{$headlineData.name}</h1>
