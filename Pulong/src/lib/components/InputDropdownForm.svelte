<script lang="ts">
	import { inputLoading } from '$lib/stores/forms';
...
<button
	...
	class={`btn variant-outline flex justify-start  dark:bg-white bg-white rounded-md dark:text-black w-full ${className}`}
>
<p class="truncate">
	{valueName}
...
<div
	class={`card  bg-white dark:bg-white shadow-xl py-2 z-10 ${cardClassName ? cardClassName : ''}`}
	...
	<div class={`${filteredItems.length > 3 ? 'h-[180px]' : ''} overflow-y-auto`}>
		<ListBox rounded="rounded-none" active="bg-blue-100  ">
			{#each filteredItems as item}
				<ListBoxItem class="" bind:group={$form[name]} {name} value={item.id}
					>
					<p class="text-gray-700 truncate  w-[310px] ">{item.name}</p>
