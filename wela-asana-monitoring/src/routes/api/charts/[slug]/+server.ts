...
export const GET: RequestHandler = async ({ params }) => {
	try {
		let data: any = {};
		const req = db.collection('charts').doc(params.slug);
...
export const PUT: RequestHandler = async ({ request, fetch, cookies, params }) => {
	try {
		...
		const reportRef = db.collection('charts').doc(params.slug);
		await reportRef.set({
			...requestBody
		});
...
export const DELETE: RequestHandler = async ({ request, fetch, cookies, params }) => {
	try {
		const res = await db.collection('charts').doc(params.slug).delete();
